class Idade:
    ...
    def Criar ( self, initial ): #cria as cartelas e seus números. Basicamente todos os números que podem ser formados por initial
        valores, flutuante = [initial], initial #flutuante não é float.
        # - - -(initial - 1 * 1 + initial + 1) - - - #
        while flutuante <= 127:
            if initial != 1: #quando initial não é 1 ele segue um padrão para a sucessão de números, segue uma sequência de somas de um que equivale a 1*initial-1 e em seguida tem um pulo de initial+1
