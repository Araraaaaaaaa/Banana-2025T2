...
class equacao:
    def __init__ ( self , A, B, C ):
        ...
        self.set__C( C )
    def set__A ( self, A ):
        if A == 0: raise ValueError("O primeiro valor não pode ser nulo")
        self.__A = A
    ...
    def R1 ( self ): 
        if self.temRaizReal() == False: return "Operação inválida"
        else: return (-self.__B - (math.sqrt(self.Delta())))/2* self.__A      
    def R2 ( self ): 
        if self.temRaizReal == False: return "Operação inválida" 
        else: return (-self.__B + (math.sqrt(self.Delta())))/2* self.__A
    def __str__ ( self ):
...
class UI:
    def main():
        ...
        while l != 0:
            ...
            l = int(input())
            match l:
                case 1: print(f"Raiz 1 = { H.R1() }")
                case 2: print(f"Raiz 2 = { H.R2() }")
                case 3: 
                ...
                case 5: print(f"Esse é o delta da sua equação do segundo grau --> {H.Delta()}")
                case 6: print(H)
                case 0: break;  
UI.main()